      ...
      <ul className='text-start'>
        {sortedData.map((user, index) => (
          <li key={user.userId}>
            ...
            : <div className='flex align-middle w-full mb-4'><p className='w-12 text-5xl text-center font-light'>{index+1}</p><p className='text-4xl/[50px] text-center '>{user.name}  スコア{user.score}</p></div>
            }
          </li>
        ))}
      </ul>
